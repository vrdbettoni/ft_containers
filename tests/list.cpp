...
#else
#define TYPE_LIST ( int, char, std::string, std::vector<int>, std::vector<std::string>, ft::list<std::string>, std::list<int> )
#endif
...
TEMPLATE_PRODUCT_TEST_CASE("list correctly copies value upon construction", "[vector][basics]", ft::list, TYPE_LIST)
{
...
TEST_CASE("sort work correctly", "[list][operations]")
{
    ...
    SECTION( "sorting with specified comparing function" ) {
        ...
        // ARRAY 2
        i = 0;
        l2.sort([](auto a, auto b) { return a == 2 || b > 42; } );
